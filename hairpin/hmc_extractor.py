...
def main():
    ...
    with open(args.output+".log","w")as file:
        print(f"CpG hmc平均水平：{results[0].split()[0]} ref总CpG位点：{ccc_CpG} 测序覆盖到的CpG位点数：{results[0].strip().split()[3]} ({result_cpg}) modC总数量：{results[0].split()[1]} unmodC总数量：{results[0].split()[2]}",file=file)
        print(f"CHG hmc平均水平：{results[1].split()[0]} ref总CHG位点：{ccc_CHG} 测序覆盖到的CHG位点数：{results[1].strip().split()[3]} ({result_chg}) modC总数量：{results[1].split()[1]} unmodC总数量：{results[1].split()[2]}",file=file)
        print(f"CHH hmc平均水平：{results[2].split()[0]} ref总CHH位点：{ccc_CHH} 测序覆盖到的CHH位点数：{results[2].strip().split()[3]} ({result_chh}) modC总数量：{results[2].split()[1]} unmodC总数量：{results[2].split()[2]}",file=file)
        if float(results[3].split()[0]) != 0 :
            print(f"CN hmc平均水平：{results[3].split()[0]}   modC总数量：{results[3].strip().split()[1]} unmodC总数量：{results[3].split()[2]}",file=file)
